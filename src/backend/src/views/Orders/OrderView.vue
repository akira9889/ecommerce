    ...
    <div class="bg-white p-5 rounded-lg shadow animate-fade-in-down">
        <div v-if="order.id">
            ...
            <table class="table-sm">
                <tbody>
                    <tr>
                        <td class="py-3 font-bold">注文番号</td>
                        <td>{{ order.id }}</td>
                    </tr>
                    <tr>
                        <td class="py-3 font-bold">注文日</td>
                        <td>{{ order.created_at }}</td>
                    </tr>
                    <tr>
                        <td class="py-3 font-bold">注文状況</td>
                        <td>
                            <div class="relative">
                                <select v-if="order.status !== 'canceled'" v-model="order.status" @change="onStatusChange">
                                    <option v-for="(status, key) in orderStatuses" :value="key" :key="key">{{ status }}
                                    </option>
                                </select>
                                <OrderStatus v-else :order="order" :orderStatuses="orderStatuses" />
                                <div class="w-8 absolute top-[-10px] left-full">
                                    <Spinner v-if="orderLoading" class="text-xs whitespace-nowrap" width="w-full"
                                        message="変更中" />
                                </div>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td class="py-3 font-bold">商品の小計：</td>
